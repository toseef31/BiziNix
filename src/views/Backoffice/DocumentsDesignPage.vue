    ...
    <div class="flex flex-col w-full" v-else>
      <div class="flex flex-col py-20 w-full px-20">
        ...
        <div class="text-black pb-4">
          Prajete si vykonať zmeny Vašich firemných údajov ? <router-link
            :to="{ name: 'CompanyDetails', params: { activeTab: 2 } }" class=" text-teal-600">Kliknite sem</router-link>
        </div>
        <div class="flex flex-row pb-10">
          <div class="flex flex-col basis-2/3">
            ...
            <div class="container mx-auto space-y-2 lg:space-y-0 lg:gap-4 lg:grid lg:grid-cols-3">
              <div class="w-full rounded hover:opacity-50 text-center" v-for="template in templates" :key="template.id">
                <img v-bind:src="template.img" v-on:click="changeTemplate(template.id)" alt="image" :class="[
                  company.doc_template_id == template.id
                    ? 'border-teal-500 border-4'
                    : 'border-2 border-gray-800',
                  '',
                ]" />
                <span v-show="company.doc_template_id == template.id"
                  class="bg-teal-500 text-gray-800 text-lg font-bold px-6 py-3 rounded">Používa sa</span>
              </div>
            </div>
          </div>
          <div class="flex flex-col px-8">
            <div class="flex flex-col basis-1/3 px-8">
              ...
              <div class="max-w-sm bg-gray-100 border border-teal-500 rounded-lg shadow justify-center">
                <img class="rounded-t-lg p-6 max-w-sm" v-bind:src="'data:image/png;base64,' + logoSrc" alt="" />
                <button
                    ...
                    <div class="flex flex-col justify-end py-2 px-4 gap-4">
                      <div class="flex flex-1/4 px-4">
                        <input
                          ...
                          id="scan" accept="image/*" v-on:change="updateLogoData($event)" name="scan" type="file" />
                      ...
                      <div class="flex flex-1/4 flex-row justify-between">
                        ...
                        <button class="bg-gray-500 hover:bg-gray-700 h-8 px-6 rounded z-10 text-gray-600"
            ...
            <div class="flex flex-col basis-1/3 px-8">
              <div class="text-2xl text-gray-600 pt-10 pb-4">
                Podpis
              </div>
              <div class="max-w-sm bg-gray-100 border border-teal-500 rounded-lg shadow justify-center">
                <img class="rounded-t-lg p-6 max-w-sm" v-bind:src="'data:image/png;base64,' + podpisSrc" alt="" />
                <button
                  class="shadow flex border py-2 w-full rounded-b-lg bg-teal-500 border-teal-500 text-gray-700 hover:text-teal-500 hover:cursor-pointer hover:bg-gray-800 space-x-2"
                  ...
                  <DialogPanel class="w-full max-w-sm rounded bg-gray-900 shadow text-white">
                    ...
                    <div class="flex flex-col justify-end py-2 px-4 gap-4">
                      <div class="flex flex-1/4 px-4">
                        <input
                          class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md text-gray-600"
                          id="scan" accept="image/*" v-on:change="updatePodpisData($event)" name="scan" type="file" />
                      </div>
                      <div class="flex flex-1/4 flex-row justify-between">
                        <button class="bg-teal-500 hover:bg-teal-700 h-8 px-6 rounded z-10 text-gray-800"
        ...
        <div class="flex flex-row gap-4">
          <div class="flex flex-col bg-gray-800 py-2 px-4 rounded-xl">
            ...
            <FormKit v-model="company.doc_sncounter_id" type="select" label="Formát čísla" :options="sncounters"
          ...
          <div class="flex flex-col bg-gray-800 py-2 px-4 rounded-xl" v-if="company.doc_sncounter_id == 3">
            ...
            <FormKit v-model="company.custom_sncounter_format" type="text" label="Formát čísla" />
