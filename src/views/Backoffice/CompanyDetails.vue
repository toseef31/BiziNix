                  ...
                  <div class="mt-5 h-0 flex-1">
                    <nav class="space-y-1 px-2">
                      <template v-for="item in navigation" :key="item.name">
                          <button @click="changeTab(item.tab)" :class="[item.tab == activeTab ? 'text-teal-500' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group w-full flex items-center pl-2 py-2 text-sm font-medium rounded-md']">
                            {{ item.name }}
                          </button>
                      </template>
                    </nav>
        ...
        <div class="hidden md:h-min-screen md:flex md:w-full">
          ...
          <div class="w-full">
              <nav class="space-y-1 px-4 py-4" aria-label="Sidebar">
                <template v-for="item in navigation" :key="item.name">
                    <button @click="changeTab(item.tab)" :class="[item.tab == activeTab ? 'text-teal-500' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'group w-full flex items-center pl-2 py-2 text-lg rounded-md font-bold']">
                      {{ item.name }}
                    </button>
                </template>
              </nav>
            ...
            <div class="w-fit flex">
              <div class="border border-gray-200 bg-gray-200 rounded-lg p-8 flex flex-row justify-between gap-4">
                <div class="flex flex-col basis-1/3 justify-center">
                  <img
                    src="@/assets/robot_stop.png"
                    class="max-w-40 self-center"
                  />
                </div>
                <div class="flex flex-col basis-2/3 justify-center">
                  <div class="text-sm">
                    ...
                    <p class="font-bold">
                      Na využívanie Bizinix služieb je potrebné, aby Vaša firma bola založená, fungujúca a mala pridelené IČO.<br>
                      <br>
