  ...
  <div class="flex flex-col px-10">
    <div class="flex flex-col py-10 items-center">
      <div class="flex flex-row gap-8 rounded-lg shadow-md p-4 bg-gray-800">
        <div class="flex flex-col">
          ...
          <label for="issue-date" class="text-white">Adresa</label>
          <FormKit type="text" name="address" v-model="address.street" />
          <div class="flex flex-row gap-4">
            <div class="flex flex-col">
              <label for="issue-date" class="text-white">PSČ</label>
              <FormKit type="text" name="psc" v-model="address.psc" />

            </div>
            <div class="flex flex-col">
              <label for="issue-date" class="text-white">Mesto</label>
              <FormKit type="text" name="city" v-model="address.city" />

            </div>

          </div>
          <label for="issue-date" class="text-white">Krajina</label>
          <FormKit type="text" name="country" v-model="address.country" />
        </div>
        <div class="flex flex-col">
          <label for="issue-date" class="text-white">IČO</label>
          <FormKit type="text" name="ico" v-model="company.ico" />
          <label for="issue-date" class="text-white">DIČ</label>
          <FormKit type="text" name="dic" v-model="company.dic" />
          <div class="flex flex-row gap-4">
            <div class="flex flex-col justify-end">
              <FormKit type="checkbox" label="Platiteľ DPH?" name="platitel" v-model="company.is_dph"
                :classes="{ label: '$reset text-sm text-white mt-1 select-none' }" />
            </div>
            <div class="flex flex-col">
              <label for="issue-date" class="text-white">DPH</label>
              <FormKit type="select"
                :options="[{ label: 'Platca DPH', value: 'platca' }, { label: 'Neplatca DPH', value: 'neplatca' }]" />
            </div>
          </div>
          <label for="issue-date" class="text-white">IČ DPH</label>
          <FormKit type="text" name="icdph" v-model="company.icdph" />
          <FormKit type="checkbox" label="Osobitná úprava dane" name="dane"
            :classes="{ label: '$reset text-sm text-white mt-1 select-none' }" />
        </div>
      </div>
    </div>
    <div class="flex flex-col">
      <div class="flex flex-row justify-between">
        <div>
          Prajete si vykonať zmeny Vašej firmy v obchodnom registry? <router-link
            :to="{ name: 'CompanyDetails', params: { activeTab: 1 } }">Kliknite sem</router-link>
        </div>
        <div>
          <button @click="updateCompany"
            class="w-fit shadow flex justify-between border items-center py-2 px-4 rounded-lg bg-teal-500 border-teal-500 text-white font-bold hover:text-teal-500 hover:cursor-pointer hover:bg-gray-800 space-x-2">
            Akualizovať údaje
          </button>
        </div>
      </div>
