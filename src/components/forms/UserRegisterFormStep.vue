<template>
  ...
  <template v-if="loading">
    <div class="flex flex-col items-center py-12">
      ...
      <span class="mt-4">Načitávam uživateľský profil...</span>
    </div>
  </template>
  <template v-else>
    <div class="grid grid-cols-2 md:grid-cols-2 gap-4 items-center">
    <FormKit type="text" :disabled="disabledInputs" name="first_name" v-model="user.first_name" label="Krstné meno" validation="required|length:2" />
  ...
  <div v-if="isValidEmail &&!isEmailUnique && !userId">
    ...
    <!-- Login inputs inside parent form -->
