<template>
  <div class="flex flex-row px-10">
    ...
    <div class="flex flex-col gap-6 basis-1/6">
      <div>
        <button @click="showModal('addBankAccount')"
  ...
  <Modal name="addBankAccount" v-model:visible="isVisible" :type="'clean'" :closable="false"
    title="Pridanie nového bankového účtu">
    <div class="bg-gray-800 rounded-lg border-teal-600 border-2">
      <div class="flex justify-end px-4 pt-4">
        <button class="bg-red-500 hover:bg-red-700 h-8 px-6 rounded text-white" v-on:click="closeModal('addBankAccount')"
          type="button">
          X
        </button>
      </div>
  ...
  <Modal name="deleteBankAccount" v-model:visible="isVisible" :type="'clean'" :closable="false"
      ...
      <div class="flex flex-row justify-center pb-4 px-4 gap-8">
        <div class="flex flex-1/4">
          <button class="bg-gray-500 hover:bg-gray-800 h-10 px-8 rounded z-10 text-white"
            v-on:click="closeModal('deleteBankAccount')">
            Nie
          </button>
        </div>
...
isVisible = setModal("deleteBankAccount", false);
...
function deleteBankAccountModal(bankAccount: any) {
  ...
  isVisible = setModal("deleteBankAccount", true);
}
