<template>
    <div role="status" class="flex py-10 h-full w-full justify-center" v-if="isLoading">
            ...
            <div class="" v-for="company in companies">
                <div class="overflow-hidden bg-white shadow sm:rounded-lg">
                    <div class="px-4 py-5 sm:px-6">
                        ...
                        <p><button @click="redirectToCompany(company.id, 'CompanyDetails')"
                                class="text-teal-500 hover:underline">Zobraziť podrobnosti</button></p>
                        <p v-if="company.fakturacia_zaplatene_do"><button @click="redirectToCompany(company.id, 'Doklady')"
                                class="text-teal-500 hover:underline font-bold">Doklady</button></p>
                        <p v-if="company.sidlo_zaplatene_do"><button @click="redirectToCompany(company.id, 'Sídlo a pošta')"
                                class="text-teal-500 hover:underline font-bold">Sídlo a pošta</button></p>
                    </div>
                </div>
            </div>
        </div>
